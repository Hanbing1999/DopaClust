...
- **Waveform Properties (1-5):**
    - `data_ttp`: Trough-to-peak duration (ms)
    - `data_rt`: Repolarization time (ms)
    - `td1s`: Total duration (ms)
    - `td2s`: Total duration at 80% amplitude (ms)
    - `hws`: Half-width (ms)
- **Burst Features (6-31):** 26 metrics describing burst firing patterns (mean, median, std, skewness, etc.).
- **Pause Features (32-57):** 26 metrics describing pause events.
- **Tonic Features (58):** Overall tonic firing rate.
- **Label (59):** `1` for photo-tagged DA neuron, `-1` for non-identified.
